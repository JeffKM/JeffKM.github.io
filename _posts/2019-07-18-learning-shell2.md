...
현대 운영체제들은 한 프로그램에서 다른 프로그램으로 빠르게 이동함으로써 하나 이상의 작업이 실행되는
듯한 환상을 심어주는 **멀티태스킹** 방식입니다. 리눅스 커널은 프로세스를 통해 이를 관리합니다.
**프로세스**란 리눅스가 CPU를 사용하기 위해 차례를 기다리는 각 프로그램을 구조화한 것입니다.

### 프로세스는 어떻게 동작하는가

시스템이 구동될 때, 커널은 몇몇 프로세스를 초기화하고 **init**이라는 프로그램을 실행합니다.
init은 차례차례 모든 시스템 서비스를 시작하기 위해 **init 스크립트**라고 불리는 쉘 스크립트(/etc에 위치에 있다)들을
실행합니다. 이 많은 서비스들은 데몬 프로그램으로 구현되어 있습니다. **데몬 프로그램**은 아무런 사용자 인터페이스 없이
백그라운드 상태로 실행됩니다. 그래서 로그인하지 않은 상태에서도 시스템은 최소한의 필요작업들을 수행합니다.
프로그램은 또 다른 프로그램을 실행할 수 있습니다. 이를 프로세스 체계에서는 **부모 프로세스**가 **자식 프로세스**를 생성한다고
표현합니다. 커널은 프로세스를 구조화한 형태로 유지하기 위해 각 프로세스의 정보를 가지고 있습니다. 각각의 프로세스는 **프로세스 ID(PID)**라고
불리는 번호를 할당 받습니다. PID는 오름차순으로 할당되고 PID 1번은 항상 init이 됩니다. 또한 커널은 실행 재개를 대기중인 프로세스도 포함하여 각 프로세스에 할당된
메모리 공간을 유지합니다. 프로세스도 파일처럼 소유자와 사용자 ID, 유효 사용자 ID 등을 가지고 있습니다.

#### ps 명령어로 프로세스 보기

다수의 프로세스를 보는 가장 일반적인 명령어는 **ps**입니다.
...
ps는 많은 정보를 보여주지 않습니다. 단지 현재 터미널 세션과 관련된 프로세스만 보여줍니다.
ps에 의해 생성된 다른 필드들에서 TTY는 teletype의 약자로 프로세스용 **제어 터미널**을
나타냅니다. TIME 필드는 프로세스의 CPU 사용 시간을 나타냅니다.
또한 x 옵션을 사용하여 ps x 명령어를 사용하면 제어되는 터미널에 상관없이 모든 프로세스를 보여줍니다.
TTY 항목의 ? 표시는 아무런 제어 터미널이 없다는 것을 가리킵니다. 이 옵션을 사용하면 소유한 모든
프로세스를 보게 됩니다. 좀 더 편하게 보기 위해 ps의 결과를 파이프를 통해 less에 전달하는 것이 도움이 될
것입니다. ps x 결과 목록에 **STAT**라는 새 항목이 추가되었습니다. **STAT**는 state의 약자로 프로세스의
현재 상태를 나타냅니다.
...
| 상태 값 | 의미 |
|:------:||
| R | 실행 상태. 프로세스는 실행 중이거나 실행 대기 중입니다. |
| S | 수면 상태. 키 입력이나 네트워크 패킷과 같은 이벤트를 기다리는 중입니다. |
| D | 인터럽트 불가능한 수면 상태. 프로세스는 입출력을 기다리는 중입니다.(디스크 입출력 등) |
| T | 종료 상태. 프로세스는 종료 요청을 받았거나 종료된 상태입니다. |
| Z | 현존하지 않거나 "좀비" 프로세스. 이것은 부모 프로세스에 의해 정리되지 않은 종료된 자식 프로세스입니다. |
| < | 높은 우선순위 프로세스. 특정 프로세스에 더 중요성을 부여하는 것이 가능합니다. 즉 CPU 시간을 더 줄 수 있습니다. |
| N | 낮은 우선수위 프로세스. 낮은 우선순위 프로세스는 더 높은 우선순위 프로세스가 사용한 뒤 프로세서 시간을 얻을 수 있습니다. |

ps aux 명령어는 더 많은 정보를 줍니다. 이 옵션 조합은 모든 사용자에 속한 프로세스들을 보여줍니다. 대시 기호 없이 옵션을 사용하는 것은
"BSD 스타일"로 명령을 실행하는 것입니다.

BSD 스타일의 ps 헤더들

| 헤더 | 의미 |
| USER | 사용자 ID. 프로세스 소유자를 나타냅니다. |
| %CPU | CPU 사용량(%) |
| %MEM | 메모리 사용량(%) |
| VSZ | 가상 메모리 크기 |
| RSS | 사용 메모리 크기. 프로세스가 사용중인 물리적 메모리량을 나타냅니다. |
| START | 프로세스가 시작된 시각. 24시간을 넘긴 값은 날짜를 사용합니다. |

#### top 명령어로 프로세스 변화 보기

ps 명령어는 실행된 순간의 상태에 대해서만 제공합니다. 시스템의 활동을 좀 더 동적으로 보기 위해서는
**top 명령어**를 사용하면 됩니다.
...
top 프로그램은 프로세스 활동순으로 나열된 시스템 프로세스들을 지속적으로 갱신하여 보여줍니다. top는 두 부분으로
나뉘어 표시합니다. 최상위에 시스템 요약과 그 아래 CPU 황동순으로 정렬된 프로세스 테이블이 표시됩니다. 가장 많이
사용하는 두 가지 키보드 명령어는 h와 q입니다. h는 프로그램 도움말 화면을 보여주는 명령어고, q는 top 프로그램을
종료합니다. 

### 프로세스 제어

프로세르를 제어해보기 위해 xlogo(or gedit or kwrite)라는 프로그램을 실험대상으로 사용할 것입니다. 
...
먼저 gedit 명령어를 입력하고 프로그램 실행을 확인하고, 그다음 터미널 창으로 돌아와 **CTRL-C**를 누르면 프로그램을 중단시킵니다.
gedit 창이 닫히고 쉘 프롬프트로 돌아갑니다. 많은 커맨드라인 프로그램들은 이 입력으로 프로그램을 중단시킬 수 있습니다.


### 프로세스를 백그라운드로 전환

gedit 프로그램의 종료 없이 쉘 프롬프트로 돌아가기를 원한다면 프로그램을 **백그라운드**로 전환하는 것으로 이를 할 것입니다. 터미널이
**포그라운드**(쉘 프롬프트처럼 화면에 표시되는 것을 가진)와 백그라운드(화면 뒤로 숨겨진 것을 가진)를 가진다고 생각하면 됩니다. 다음처럼
&기호를 명령어오 함께 사용하면 프로그램을 실행 즉시 백그라운드로 이동합니다.
...
[me@linuxbox ~]$ 
```
명령어 입력 후에 gedit 창이 나타나고 쉘 프롬프트로 돌아옵니다. 하지만 숫자들도 함께 출력됩니다. 이 메시지는 **작업 제어**라고 부르는
쉘 기능의 일부입니다. 쉘은 이 메시지로 PID가 28236인 1번 작업이 시작되었다는 것을 알려줍니다. ps를 실행하면 이 프로세스를 볼 수 있습니다.
또한 쉘의 작업 제어 시스템은 터미널에 실행 중인 작업을 나열해줍니다. Jobs 명령어를 사용하면 다음과 같은 목록을 볼 수 있습니다.
...
백그라운드에 있는 프로세스는 CTRL-C를 통한 프로세스 중단을 포함해 어떤 키보드 입력도 영향을 받지 않습니다. 프로세스를 포그라운드로 전환하기 위해서는
**fg 명령어**를 사용합니다. 
...
**fg 명령어**는 퍼센트 기호(%)와 작업 번호(jobspec)를 사용합니다. 여기서 gedit를 종료하려면 CTRL-C를 누르면 됩니다.

### 프로세스 정지(일지 정지)

때때로 프로세스를 종료하지 않고 멈추고 싶을 때가 있습니다. 이것은 종종 포그라운드 프로세스를 백그라운드로 이동하기 위해 행해집니다.
포그라운드 프로세스를 정지하기 위해서는 **CTRL-Z**를 누르면 됩니다.

### 시그널

#### kill로 시그널 보내기

**kill 명령어**는 프로세스를 종료하기 위해 사용됩니다. 이는 비정상적으로 동작하거나 그렇지 않으면 종료를 거부하는 프로그램의 실행이 끝나게
해줍니다.
...
[1]+ Terminated                        gedit
```
정확히 kill 명령어는 프로세스를 종료하지는 않습니다. 오히려 프로세스에 시그널을 보낸다는 표현이 옳습니다. 시그널은 운영체제가 프로그램들과 통신하기
위한 여러 방법 중 하나입니다. 프로그램들은 결국 시그널을 귀 기울이고 있다가 받은 시그널에 따라 행동하게 될 것입니다. 프로그램이 시그널을 들을 수 있고
그에 따라 행동한다는 사실은 종료 시그널을 받았을 때 진행 중인 작업을 저장하는 것과 같은 동작이 가능하다는 것을 나타냅니다.

> kill [-signal] PID ...

커맨드라인에 지정된 시그널이 없다면, 기본적으로 TERM 시그널을 보냅니다.

주요 시그널

|번호|이름|의미|
|:--:|:--:|:--:|
|1|HUP|Hang up. 제어 터미널과 "연결이 끊어진" 프로그램을 가리키는 데 사용됩니다.|
|2|INT|Interrupt. 프로그램은 항상 종료합니다.|
|9|KILL|Kill. Kill 시그널은 실제로 해당 프로그램에 보내지지 않습니다. 오히려 커널이 즉시 프로세스를 종료시킵니다.|
|15|TERM|Terminate. 이 신호를 보냈을 때 프로그램이 여전히 시그널을 받을 수 있을 정도로 "살아있다면" 프로그램은 종료될 것입니다.|
|18|CONT|Continue. Stop 시그널로 정지된 프로세스를 복원합니다.|
|19|STOP|이 시그널은 프로세스를 종료 없이 정지시킵니다. 해당 프로세스에 직접 보내지 않으므로 무시할 수 없습니다.|

#### killall로 다수의 프로세스에 시그널 보내기

**killall 명령어**를 사용하면 명시된 프로그램 또는 사용자 이름과 일치하는 다수의 프로세스에 시그널을 보내는 것도 가능합니다.
